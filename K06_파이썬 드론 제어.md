...
#### 1) 삼바로 위 예제 코드를 라즈베리파이로 복사한다. 

#### 2) 예제의 코드에서 aidrone.Open("COM3") ==>  aidrone.Open("/dev/serial0") 으로 수정한다. 

#### 3) putty에서 수정된 예제를 실행해서 드론 자제적으로 비행 제어를 할 수 있다. (예제 중  move, velocity, rotation, alititude, altitude_mode 등)
